...
let JobApplicationService = class JobApplicationService {
    ...
    async create(file, createApplicationDto) {
        try {
            ...
            throw new common_1.HttpException('Failed to upload file or save application', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    async findAll() {
    ...
    async remove(id) {
        ...
        const publicId = application.resume.substring(application.resume.lastIndexOf('/applicants/') + 1, application.resume.lastIndexOf('.'));
        try {
